  ...
  useEffect(() => {
    ...
    intervalRef.current = window.setInterval(() => {
      setAirQualityData((currentData) => {
        ...
        (Object.keys(newPollutants) as Array<keyof PollutantData>).forEach(
          (key) => {
            const fluctuation = Math.random() * 4 - 2; // -2 to +2
            newPollutants[key] = Math.max(
              0,
              Math.round(newPollutants[key] + fluctuation)
            );
          }
        );
        ...
        const dominantPollutantEntry = Object.entries(newPollutants).reduce(
          (a, b) => (a[1] > b[1] ? a : b)
        );
        const dominantPollutantKey =
          dominantPollutantEntry[0] as keyof PollutantData;
        const newDominantPollutant =
          POLLUTANT_INFO[dominantPollutantKey]?.name || "N/A";
      ...
      <div className="w-full max-w-4xl mx-auto">
        <header className="text-center mb-6">
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-shadow">
            Auracast - Air Quality
          </h1>
          <p className="text-lg text-slate-700 mt-2">
            Check the air you breathe, anywhere in the world.
          </p>

          <nav className="flex justify-center space-x-2 mt-4">
            <button
              className="px-4 py-2 text-slate-600 font-medium hover:text-indigo-600 transition-colors rounded-full hover:bg-black/5 "
            ...
            <button
              className="px-4 py-2 text-slate-600 font-medium hover:text-indigo-600 transition-colors rounded-full hover:bg-black/5 "
              onClick={() => window.open("./community/aura_community.html")}
            >
              COMMUNITY
            </button>
            <br />
            <button
              className="px-4 py-2 text-slate-600 font-medium hover:text-indigo-600 transition-colors rounded-full hover:bg-black/5 "
              onClick={() => window.open("./symptomTracker/symptom.html")}
            >
              SYMPTOM TRACKER
            </button>
            <br />
